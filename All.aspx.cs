...
public partial class All : System.Web.UI.Page
{
        ...
        if(!IsPostBack)
        {
            using (MySqlConnection conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MyConnectionString"].ConnectionString))
            {
                using (MySqlCommand sample = new MySqlCommand())
                {
                    ...
                    sample.CommandText = "Select Q_Title , Q_Description from querydata";
    ...
    private void AddDataToPanel(string Heading , string Description , int count)
    {
        ...
        secondDiv.Attributes.Add("class","col-sm-10");
        thirdDiv.Attributes.Add("class","ibox");
        fourthDiv.Attributes.Add("class","ibox-title");
        fifthDiv.Attributes.Add("class","ibox-content");
        fourthDiv.Style["height"] = "auto";
        //fourthDiv.Style["overflow"] = "auto";
        //fifthDiv.Style["overflow"] = "auto";
        

        QueryPanel.Controls.Add(firstDiv);
       ...
       // fourthDiv.Controls.Add(DescriptionLabel);
        ...
        if (IsPostBack)
        {
            using (MySqlConnection conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["MyConnectionString"].ConnectionString))
            {
                using (MySqlCommand sample = new MySqlCommand())
                {
                    sample.Connection = conn;
                    sample.CommandType = CommandType.Text;
                    sample.CommandText = "Select Q_Title , Q_Description,Q_Type from querydata WHERE Q_Type = @Type";
